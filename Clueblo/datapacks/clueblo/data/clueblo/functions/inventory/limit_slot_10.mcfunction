...
execute if score @s salon_S matches 1 run item replace entity @s container.10 with minecraft:clock{filler10b:1b,CustomModelData:2,display:{Name:'[{"translate":"clueblo.room.salon","color":"gold"}]'}} 1
execute if score @s salon_S matches 0 run item replace entity @s container.10 with minecraft:clock{filler10b:1b,CustomModelData:50,display:{Name:'[{"translate":"clueblo.room.salon","color":"gold","strikethrough":"true"}]'}} 1
