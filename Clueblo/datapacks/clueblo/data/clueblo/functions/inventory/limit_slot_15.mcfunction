...
execute if score @s bill_S matches 1 run item replace entity @s container.15 with minecraft:clock{filler15b:1b,CustomModelData:7,display:{Name:'[{"translate":"clueblo.room.billiard","color":"gold"}]'}} 1
execute if score @s bill_S matches 0 run item replace entity @s container.15 with minecraft:clock{filler15b:1b,CustomModelData:50,display:{Name:'[{"translate":"clueblo.room.billiard","color":"gold","strikethrough":"true"}]'}} 1
